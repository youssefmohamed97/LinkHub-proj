...
const http$1 = require('./utils/http.js');
...
class Http  {
   ...
   */
   setupOnce(
    ...
    utils.fill(http, 'get', wrappedHttpHandlerMaker);
    utils.fill(http, 'request', wrappedHttpHandlerMaker);
    ...
    if (NODE_VERSION.major && NODE_VERSION.major > 8) {
      ...
      utils.fill(https, 'get', wrappedHttpsHandlerMaker);
      utils.fill(https, 'request', wrappedHttpsHandlerMaker);
    }
  }
}Http.__initStatic();
...
function _createWrappedRequestMethodFactory(
  ...
  return function wrappedRequestMethodFactory(originalRequestMethod) {
    return function wrappedMethod( ...args) {
      const requestArgs = http$1.normalizeRequestArgs(httpModule, args);
      const requestOptions = requestArgs[0];
      const requestUrl = http$1.extractUrl(requestOptions);

      // we don't want to record requests to Sentry as either breadcrumbs or spans, so just use the original method
      if (http$1.isSentryRequest(requestUrl)) {
      ...
      return originalRequestMethod
        ...
        .once('response', function ( res) {
          ...
          if (requestSpan) {
            ...
            requestSpan.description = http$1.cleanSpanDescription(requestSpan.description, requestOptions, req);
            requestSpan.finish();
          }
        })
        .once('error', function () {
